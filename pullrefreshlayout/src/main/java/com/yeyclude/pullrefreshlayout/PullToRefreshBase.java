...
public abstract class PullToRefreshBase <T extends View> extends LinearLayout implements IPullToRefresh<T>{
    ...
    /** 表示是否消费了touch down事件，如果是，则不执行父类的onTouchEvent方法 */
    private boolean mDoDownTouchEvent = false;
    ...
    /** 监听刷新视图自动滑动监听 */
    private RefreshViewAutoScrollListener mRefreshViewAutoScrollListener;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mHeaderViewHeight = (int) (getResources().getDisplayMetrics().density * 60);
    ...
    private void refreshLoadingViewsSize() {
        ...
        /**
         * 这里得到Header和Footer的高度，设置的padding的top和bottom就应该是header和footer的高度，因为header和footer是完全看不见的
         */
        ...
        /**
         * paddingtop和paddingbottom取负值，使header和footer不可见
         */
     ...
     *
     */
    public interface RefreshViewAutoScrollListener {
        /**
         * 视图正在自动滑动
         *
         * @param mScrollFromY 滑动开始Y
         * @param mScrollToY 滑动到Y
         * @param mCurrentY 滑动当前Y
         * @param deltaY 距上次的滑动距离Y
         */
        public void slidingY(int mScrollFromY, int mScrollToY, int mCurrentY, int deltaY);
    ...
    public void setRefreshViewAutoScrollListener(RefreshViewAutoScrollListener l) {
        this.mRefreshViewAutoScrollListener = l;
    }
    ...
    public void setPage(int page) {
        ((HeaderLoadingLayout) mHeaderLayout).setPage(page);
    }
    ...
    public void setHeadView(View view) {
        ((HeaderLoadingLayout) mHeaderLayout).setHeadView(view);
    }
     ...
     * @param x
     * @param y
     * @return true = 横向，false = 竖向
     */
    private boolean cannotScrollinRect(float x, float y){
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        //横向滑动不处理
        if(cannotScrollinRect(event.getRawX(), event.getRawY())) {
            return false;
        }
        ...
        processMotionEvent(event, action);

        return mIsHandledTouchEvent;
    }

    private void processMotionEvent(MotionEvent event, final int action) {
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                mDoDownTouchEvent = true;
                mLastMotionY = event.getY();
                mLastMotionX = event.getX();
                mIsHandledTouchEvent = false;
                scrollY = -getScrollYValue();
                // if (State.RELEASE_TO_REFRESH != mHeaderLayout.getState())
                // {
                // scrollY = 0;
                // }
                break;
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                final float adsDiffX = Math.abs(deltaX);
                // 这里有三个条件：
                ...
                if (absDiff > mTouchSlop || isPullRefreshing() || isPullLoading()) {
                    ...
                    if (isPullRefreshEnabled() && isReadyForPullDown() && adsDiffX < absDiff) {
                        ...
                        // 如果截断事件，我们则仍然把这个事件交给刷新View去处理，典型的情况是让ListView/GridView将按下Child的Selector隐藏
    ...
    /**
     * 是否消费touch down事件，如果true，则不执行父类的onTouchEvent方法
     * @param ev 事件
     * @return true = 消费事件，不向父类传递事件；false = 不消费事件，向上一层父类传递事件
     */
    @Override
    public final boolean onTouchEvent(MotionEvent ev) {

        if (!mDoDownTouchEvent && ev.getAction() != MotionEvent.ACTION_DOWN) {
            return true;
        }
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                scrollY = -getScrollYValue();
                float scale = Math.abs(scrollY) / (float) mHeaderHeight;
    ...
    protected void pullHeaderLayout(float delta) {
        ...
        // if (scrollY + delta > mHeaderHeight)
        // {
        // scY = (int) (mHeaderHeight - scrollY);
        // }
        ...
        // ((HeaderLoadingLayout)
        // mHeaderLayout).showProgressImage((int)(delta/mHeaderHeight));
        ...
        {
            if (scrollY >= mHeaderViewHeight )//- ViewUtil.getInstance().getScaleNum(20)
            {
    ...
    protected void resetHeaderLayout() {
         ...
         * 下拉尺寸不足视图高度，类似弹簧缩回
         */
        if (refreshing && scrollY < mHeaderViewHeight ) {
            // scrollY = 0;
            smoothScrollTo(0);
            // 因为滚动回原始位置的时间是200，我们需要等回滚完后才执行刷新回调
            postDelayed(new Runnable() {
         ...
         */
        if (refreshing) {
            // 测试提出松开正在刷新时，也应该显示pull to refresh的字样
    ...
    public void setRefreshTime(CharSequence charSequence){
        if (null != mHeaderLayout) {
            mHeaderLayout.refreshingSuccess(charSequence);
        }
    }
    ...
    final class SmoothScrollRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mStartTime == -1) {
                ...
                //float scale = Math.abs(mCurrentY) / (float) mHeaderHeight;
                // mHeaderLayout.onPull(scale);
                setScrollTo(0, mCurrentY );
                Log.d(TAG, mHeaderLayout.getPaddingBottom() + "" + mHeaderLayout.getPaddingTop());
                if (mRefreshViewAutoScrollListener != null) {
                    mRefreshViewAutoScrollListener.slidingY(mScrollFromY, mScrollToY, mCurrentY, deltaY);
                }
            }
            ...
            if (mContinueRunning && mScrollToY != mCurrentY) {
                ...
                if (mRefreshViewAutoScrollListener != null) {
                    mRefreshViewAutoScrollListener.end();
                }
            }
        }
